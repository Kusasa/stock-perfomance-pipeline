...
echo "Create and subscribe to SNS Alert"
...
echo "Create authentication key"
...
echo "Capture output values from the stack deployment as variables"
EC2PublicIP=$(aws cloudformation describe-stacks --stack-name stock_perfomances_infra --query "Stacks[0].Outputs[?OutputKey=='EC2PublicIP'].OutputValue" --output text)
RDSInstanceEndpoint=$(aws cloudformation describe-stacks --stack-name stock_perfomances_infra --query "Stacks[0].Outputs[?OutputKey=='RDSInstanceEndpoint'].OutputValue" --output text)
RDSInstanceIdentifier=$(aws cloudformation describe-stacks --stack-name stock_perfomances_infra --query "Stacks[0].Outputs[?OutputKey=='RDSInstanceIdentifier'].OutputValue" --output text)

echo "Connect to the EC2 instance using SSH and create the home directory of the pipeline"
ssh -i "stock_perfomance_key.pem" ubuntu@$EC2PublicIP "mkdir -p ~/pipeline ~/pipeline/dags ~/pipeline/logs ~/pipeline/plugins"

echo "Copy dag folder with its contents to the dags folder in the remote server"
scp -i "stock_perfomance_key.pem" -r ./dag ubuntu@$EC2PublicIP:~/pipeline/dags

echo "Connect to the remote server, install docker engine and deploy the airflow docker containers with its required libraries"
...
echo "Connect to the airflow-webserver-1 container and install necessary packages"
...
echo "Set airflow connections and variables"
