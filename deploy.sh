...
chmod 600 ~/.ssh/stock_perfomance_key.pem
...
KeyPairID=$(aws cloudformation describe-stacks --stack-name stockPerfomancesInfra --query "Stacks[0].Outputs[?OutputKey=='KeyPairID'].OutputValue" --output text)
...
# Connect to the remote server, install docker engine and deploy the airflow docker containers with its required libraries
ssh -i "~/.ssh/stock_perfomance_key.pem" ubuntu@"${EC2PublicIP}" << 'EOF'
sudo apt-get update &&
...
# Connect to the airflow-webserver-1 container and install necessary packages
ssh -i "~/.ssh/stock_perfomance_key.pem" ubuntu@"${EC2PublicIP}" << 'EOF'
...
# Set airflow connections and variables
